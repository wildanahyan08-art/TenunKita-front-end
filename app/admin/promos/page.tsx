...
export default function PromosPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/promos`, {
      ...
      .then((result) => {
        const data: Promo[] = Array.isArray(result) ? result : (result.data || []);
        setPromos(data);
        setIsLoading(false);
      })
      .catch(() => {
  ...
  const totalUsage = promos.reduce((sum, p) => sum + p.usedCount, 0);
    ...
    return (
      <div className="min-h-screen bg-[#faf6f0] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-amber-800 font-serif text-lg">Memuat promo...</p>
    ...
    return (
      <div className="min-h-screen bg-[#faf6f0] flex items-center justify-center p-6">
        <div className="bg-white rounded-3xl shadow-xl p-10 max-w-md w-full text-center border border-amber-200/60">
          ...
          <p className="text-gray-500 text-sm mb-8">Terjadi kesalahan saat mengambil data promo</p>
          <button
  ...
  return (
    <div className="min-h-screen bg-[#faf6f0]">
        ...
        <div className="relative max-w-7xl mx-auto px-4 md:px-8 py-10 md:py-14">
          <div className="text-center">
            ...
            <p className="text-amber-400/60 font-serif italic text-sm md:text-base mt-2">
              Manajemen kode promo dan diskon TenunKita
            </p>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-6 md:py-10 pt-24 md:pt-28">
        {/* Stats ringkasan */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <div className="bg-white rounded-xl border border-amber-200/40 p-5">
            <p className="text-2xl md:text-3xl font-bold text-[#1a120b]">{promos.length}</p>
            <p className="text-[11px] text-gray-500 font-semibold uppercase tracking-[0.15em] mt-1">Total Promo</p>
          </div>
          <div className="bg-white rounded-xl border border-amber-200/40 p-5">
            <p className="text-2xl md:text-3xl font-bold text-[#1a120b]">{activePromos}</p>
            <p className="text-[11px] text-gray-500 font-semibold uppercase tracking-[0.15em] mt-1">Promo Aktif</p>
          </div>
          <div className="bg-white rounded-xl border border-amber-200/40 p-5">
            <p className="text-2xl md:text-3xl font-bold text-[#1a120b]">{totalUsage}</p>
            <p className="text-[11px] text-gray-500 font-semibold uppercase tracking-[0.15em] mt-1">Total Penggunaan</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-gray-500">
              <strong className="text-[#1a120b]">{filtered.length}</strong> promo ditemukan
            </span>
          </div>
        ...
        {filtered.length === 0 ? (
          <div className="bg-white rounded-2xl border border-amber-200/40 p-12 text-center">
            ...
            <p className="text-gray-500 font-serif text-lg">
              {search ? 'Tidak ada promo yang cocok' : 'Belum ada promo'}
            </p>
            {!search && (
              <Link
                href="/admin/promos/add"
                className="inline-flex items-center gap-2 mt-4 px-5 py-2.5 bg-amber-700 hover:bg-amber-600 text-white font-medium rounded-xl transition-all shadow-sm text-sm"
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>
                </svg>
          ...
          <div className="bg-white rounded-2xl border border-amber-200/40 overflow-hidden shadow-sm">
            ...
            <div className="hidden md:grid grid-cols-12 gap-4 px-6 py-3.5 bg-amber-50/80 border-b border-amber-200/40 text-[11px] font-bold uppercase tracking-[0.15em] text-gray-500">
              ...
              <div className="col-span-1 text-center">Pemakaian</div>
                  ...
                  >
                    ...
                    <div className="md:hidden flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-[#1a120b] text-sm">{promo.code}</p>
                        <p className="text-gray-500 text-xs mt-0.5 truncate">{promo.description || '—'}</p>
                    ...
                    <div className="hidden md:flex col-span-2 items-center gap-3">
                      ...
                      <span className="text-sm font-bold text-[#1a120b] truncate">{promo.code}</span>
                    </div>
                    ...
                    <div className="hidden md:flex col-span-1 items-center justify-center text-sm">
                      <span className={`font-semibold ${remaining <= 0 ? 'text-red-600' : 'text-gray-700'}`}>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
            <div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full border border-amber-200/60">
              <div className="text-center">
                ...
                <h3 className="text-lg font-bold text-[#1a120b] mb-2">Hapus Promo</h3>
                <p className="text-sm text-gray-500 mb-6">Apakah Anda yakin ingin menghapus promo ini? Tindakan ini tidak dapat dibatalkan.</p>
                <div className="flex items-center gap-3">
                  <button
...
function formatValue(promo: Promo) {
  ...
  return new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(promo.value);
}
